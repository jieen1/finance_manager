...
class HoldingTest < ActiveSupport::TestCase
  ...
  test "calculates average cost basis" do
    ...
    # Weighted average: (10*212 + 15*216) / 25 = 214.4, (5*128 + 30*124) / 35 = 124.57...
    assert_equal Money.new((10 * 212.00 + 15 * 216.00).to_d / 25), @amzn.avg_cost
    assert_equal Money.new((5 * 128.00 + 30 * 124.00).to_d / 35), @nvda.avg_cost
  end
  ...
  test "calculates average cost basis from another currency" do
    ...
    # Weighted average
    assert_equal Money.new((10 * 212.00 + 15 * 216.00).to_d / 25, "CAD").exchange_to("USD", fallback_rate: 1), @amzn.avg_cost
    assert_equal Money.new((5 * 128.00 + 30 * 124.00).to_d / 35, "CAD").exchange_to("USD", fallback_rate: 1), @nvda.avg_cost
  end
