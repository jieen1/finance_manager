class Provider::GenericOpenai < Provider
  include LlmConcept
  ...
  def initialize(api_key, api_endpoint:, models: [])
    @available_models = models
    @client = ::OpenAI::Client.new(
      access_token: api_key,
      uri_base: api_endpoint.end_with?("/") ? api_endpoint : "#{api_endpoint}/"
    )
  end
  ...
  def auto_categorize(transactions: [], user_categories: [])
    raise Error, "Auto-categorize is not supported for generic OpenAI-compatible providers"
  end

  def auto_detect_merchants(transactions: [], user_merchants: [])
    raise Error, "Auto-detect merchants is not supported for generic OpenAI-compatible providers"
  end
      ...
      if streamer.present?
        ...
        stream_proc = proc do |chunk, _bytesize|
          ...
          if finish_reason.present?
            ...
            fn_requests = accumulated_tool_calls.values.map do |tc|
